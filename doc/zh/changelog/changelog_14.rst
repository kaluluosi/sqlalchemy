...
.. changelog::
    ...
    .. change::
        ...
        修复了关于缓存的重要问题，组合：func:`_orm.aliased()`和 :func:`_hybrid.hybrid_property`表达式组成会导致缓存键不匹配，从而导致缓存键保留了实际 :func:`_orm.aliased` 对象，同时也无法匹配等效结构，填充/占用缓存。
...
.. changelog::
    ...
    .. change::
        ...
        修复了一个错误/回归，即在使用 :func:`.bindparam()` 与相同名称的列在 :meth:`.Update.values` 方法和2.0中的 :meth:`_dml.Insert.values` 方法中的某些情况下，会在默默中失败，不遵循其中的SQL表达式 , 用参数表达式替换表达式，例如SQL函数等等。特定的情况是针对ORM实体构建的语句，而不是纯 :class:`.Table`实例，但是如果使用 :class:`.Session`或:class:`.Connection`调用语句，则会发生。
...
.. changelog::
    ...
    .. change::
        ...
        添加 :paramref:`.FunctionElement.column_valued.joins_implicitly` 参数，该参数有助于防止在使用表值函数或列值函数时出现“笛卡尔积”警告。该参数已经在 :ticket:`7845` 中为 :meth:`.FunctionElement.table_valued`引入了，但是对于 :meth:`.FunctionElement.column_valued` 也未添加。
    ...
    .. change::
        ...
        修复了使用包括 :meth:`_types.TypeEngine.bind_expression`的表达式的类型时，当与一个 “扩展”(即 “IN”)参数结合使用时可能失败，例如与字面值绑定时使用的那些参数表达式Binds的参数，例如SQLite，asyncpg，MySQL，Oracle等。一些编译的表单将不能正确地维护参数顺序，例如PostgreSQL `` regexp_replace()``函数，在用于 :ticket:`4123`首次引入的 :class:`.CTE`结构的“嵌套”功能中，以及使用Oracle的 :meth:`.FunctionElement.column_valued` 方法形成的可选择表。
...
.. changelog::
    ...
    .. change::
        ...
        从 :class:`_asyncio.AsyncResult`中删除不起作用的``merge()``方法。此方法从未起作用，并且错误地包含在中 :class:`_asyncio.AsyncResult`。
    ...
    .. change::
        ...
        场景是当:class:`_engine.Connection`已在返回连接时在其DBAPI连接上发出``.rollback（）``时，如果其指示连接池放弃执行自己的“重置”以节省附加 方法调用。但这将防止使用自定义重置方案从该挂钩中使用，因为这些挂钩本质上不仅仅是调用 ``.rollback()``，而且需要在所有情况下调用。这是版本1.4中出现的回归。
...
-修改了 DeclarativeMeta 元类。这样，自定义基类中加入新的属性应该可以在 ORM 中使用。.. 更新日志::
    ...
    .. change::
        ...
        修复了 ORM 回归问题，其中针对现有 :func:`_orm.aliased` 建构的 :func:`_orm.aliased` 方法调用将在允许情况下失败，并且如果将原始 :func:`_orm.aliased` 构造物仅针对现在正在替换的表，则原始 :func:`_orm.aliased` 构造物将被忽略。如果没有可选择的参数构建 :func:`_orm.aliased` 并在 :func:`_orm.aliased` 中使用 :func:`_orm.aliased`以生成子查询的别名时，则具有针对子查询的外部 :func:`_orm.aliased` 对象的嵌套行为将保留不变。
...
.. change::
    ...
    :class:`_asyncio.AsyncSession`现在支持覆盖将用作代理实例作为proxied instance的:class:`_orm.Session`。
...
.. changelog::
    ...
    .. change::
        ...
        修复了在版本1.4.21 / :ticket:`6752`中添加的:meth:`_sql.HasCTE.add_cte`功能中，如何处理来自超类的`__tablename__`例程的一列的问题。

    .. change::
        :tags: bug, PostgreSQL, SQLAlchemy ORM
        :tickets: 6752

        PostgreSQL的:class:`_postgresql.ENUM`数据类型是原生的，因此不应使用``native_enum=False``标志。
    ...
    .. change::
        ...
        :class:`_asyncio.AsyncSession`现在支持覆盖将用作代理实例的:class:`_orm.Session`。
        可以使用:paramref:`~.AsyncSession.sync_session_class`参数传递自定义的``Session``类，
        或通过子类化``AsyncSession``并指定自定义的:attr:`~.AsyncSession.sync_session_class`来指定。 
...
.. changelog::
    ...
    .. change::
        ...
        在 :term:`2.0 风格`中使用的 ORM 现在可以通过将构造体提供给 ORM 上下文中的 :meth:`_sql.Select.from_statement` 方法，从由 UPDATE..RETURNING 或 INSERT..RETURNING 语句返回的行中返回 ORM 对象。
    ...
    .. change::
        ...
        将“future”关键字添加到已知于:func:`_sa.engine_from_config`函数的单词列表中，以便在使用诸如``sqlalchemy.future = true``或``sqlalchemy.future = false``这样的关键字时可以将“true”和“false”配置为“boolean”值。
    ...
    .. change::
        ...
        此选项将在 :class:`_engine.Connection`执行语句时，将一个额外的每条消息令牌添加到日志消息中。这个标记本身不是日志名的一部分（该部分可以通过现有的:paramref:`_sa.create_engine.logging_name`参数来改变)
    ...
    .. change::
        ...
        已为:class:`_schema.Column`对象和ORM实体属性添加了支持，作为传递给:meth:`_postgresql.Insert.on_conflict_do_update` 和 :meth:`_sqlite.Insert.on_conflict_do_update`方法的“set_”字典的键，与该目标中的:class：`_schema.Column`对象相匹配的“c”集合。以前，只期望字符串列名，假定列表达式是不在表格之外的表达式，它将随附一个警告而完全呈现。
    ...
    .. change::
        ...
        已对:class:`_sql.Sequence` 和 :class:`_sql.Identity`对象中的``cycle = False``和``order = False``进行了正确呈现，将其呈现为“NO CYCLE”和“NO ORDER”。

    .. change::
        :tags: schema, usecase
        :tickets: 2843

        为:class:`_ddl.CreateTable`, :class:`_ddl.Drop-t-able`, :class:`_ddl.CreateIndex`和:class:`_ddl.DropIndex`构造添加了 :paramref:`_ddl.CreateTable.if_not_exists`,
    ...
    .. changelog::
        ...
        .. change::
            ...
            :meth:`_reflection.Inspector.reflecttable`已重命名为:meth:`_reflection.Inspector.reflect_table`。
    ...
    .. change::
        ...
        现在可以将操作为:func:`.true` and :func:`.false`的运算符应用于在不支持“本地布尔”表达式的后端上进行的: func: `_expression.join`的"onclause"中，并且该表达式将为真返回“1 = 1”，而为假则为“1 = 0”。类似地，十多年前开发了 :ticket:`2804` 的“逻辑与或”表达式就是这样。支持的后端包括SQLite，PostgreSQL，MySQL/MariaDB和Oracle。
    ...
    .. change::
        ...
        :class:`~_engine.URL`对象现在是一个不可变的命名元组。要修改URL对象，请使用:class:`~_engine.URL.set`方法生成新的URL对象。
    ...
    .. change::
        ...
        :func:`.true`和:func:`.false`运算符现在可以应用于JOIN的“onclause”，并呈现为“1 = 1”和“1 = 0”。
    ...
    .. change::
        ...
        :class:`_schema.Table`参数“mustexist”已重命名为:paramref:`_schema.Table.must_exist`，并发出警告。
